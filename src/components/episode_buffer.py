...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        ...
        scheme.update({
            "filled": {"vshape": (1,), "dtype": np.int32},
        })
        ...
        for field_key, field_info in scheme.items():
            ...
            if group:
                ...
                shape = vshape
            if episode_const:
                self.data.episode_data[field_key] = np.zeros((batch_size, *shape), dtype)
            else:
                self.data.transition_data[field_key] = np.zeros((batch_size, max_seq_length, *shape), dtype)
    ...
    def update(self, data, bs=slice(None), ts=slice(None), mark_filled=True):
        ...
        for k, v in data.items():
            ...
            if k in self.preprocess:
                ...
                target[new_k][_slices] = v.reshape(target[new_k][_slices].shape)
